...
def upload_dashboard(
    ...
    cloudfront_distribution_id: Optional[str],
    ...
    if cloudfront_distribution_id:
        try:
            cf = boto3.client("cloudfront", **kwargs)
            ...
            logger.info(f"[CF] Invalidation submitted for {cloudfront_distribution_id}")
        except (BotoCoreError, ClientError) as exc:
            logger.warning(f"[CF] Cache invalidation failed: {exc}")
        except Exception as exc:
            logger.warning(f"[CF] Unexpected error during cache invalidation: {exc}")
